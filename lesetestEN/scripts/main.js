...
$(document).ready(function() {	
	...
	var submitString = 'http://vox-wp2.vox.no/dataoppsamling/lesetest/submituser.php?nocache='+Math.random();
...
function startTestIntro(){
	...
	$("#intro-content").html("<span class='intro-sign'></span><span class='intro-text'><p>In this test, your task is to read short texts without using any aid. Click on the word that you think fits best to the text.  You can alter your choice by choosing one of the other words. Only one alternative is correct.</p></span><div class='task intro'><h2>Knee damage</h2><p>Anders hurt his knee during a ski accident. In a week he will be operated.  After the operation, he will need to train up the <span class='options'><span>forearm </span><span class='selected'>knee </span><span>brain </span><span>feet</span></span>. His physiotherapist advises him to use <span class='options'><span>stilts</span><span class='selected'>crutches</span><span>skate</span><span>rollerskis</span></span>&nbsp;for 3 months.</p></div><span class='intro-text'><p>Her you had four alternatives to choose from in each sentence, and the words knee and crutches were chosen because they fitted best.</p>  </span>");
...
function submitItem(){	
    ...
    var submitString = 'http://vox-wp2.vox.no/dataoppsamling/lesetest/submititem.php?uid=' + uid + '&itemid=' + submitArray[0] +'&answer='+ submitArray[1] +'&correctanswer=' + submitArray[2] +'&correct='+ submitArray[3] +'&time='+ taskTime +'&totaltime='+ (new Date().getTime()-testStartTime) +'&ver='+ xmlVersion + '&timeout=' + testOver + '&nocache=' + Math.random();
...
function drawAnswers() {
    ...
    for(var i=0; i < $XML.find("startest").find('opgave').length; i++) {
		...
		var questionWrapper = '<div class="question-wrapper examination"><div class="question task">';

		var pArray = $XML.find("startest").find('opgave').eq(i).children();
		...
		for(var j=0; j < pArray.length; j++) {
			if(pArray[j].nodeName.toLowerCase()=="overskrift"){
				...
				questionWrapper +="<p>"
				var itemArray = $XML.find("startest").find('opgave').eq(i).children().eq(j).children();				
				var lastTextWasItem=false;
				...
				for(var k=0; k<itemArray.length; k++){
					if(itemArray[k].nodeName.toLowerCase() == "tekst"){
						...
						var currentItemArray = $(itemArray[k]).text().split(',');
						...
						for(var l=0; l < currentItemArray.length; l++){
							...
							if(selectedAnswerArray[questionIndex] == l+1) {
								...
								if(selectedAnswerArray[questionIndex] != currentItemCorrectIndex ) {
							...
							questionWrapper += "<span class='"+optionClass+"'>"+checkmark+currentItemArray[l]+"</span>";
						}
						questionIndex++;
		...
		questionWrapper += "<div class='mark "+ correctClass +"'>" + correctMark + "</div>";
...
function setupResult(){
	...
	$("#result .score").html("YOUR RESULT:<span>"+rapportScore+"</span>");

	var rapportScoreBTOA = window.btoa(rapportScore);


	if(finalScore >= 4){//grøn
		$(".feedback.reading h2").html("Your result is over 4");
		$(".feedback.reading p").html("Results over 4 show that you are a good reader. You are able to read most texts both fast and precise.");
	...
	}else if(finalScore >= 3){//GUL
		$(".feedback.reading h2").html("Your result is between 3 and 4.");
		$(".feedback.reading p").html("The result that you achieved shows that your reading skills are quite good. However, some texts can be difficult and take a longer time for you to read.");
		...
		$(".feedback.reading h2").html("Your result is between 0 and 3");
		$(".feedback.reading p").html("The result that you achieved shows that you may have difficulties reading a great amount of texts and that reading itself can take a longer time for you. It can be useful for you to spend more time on reading. ");
