...
$(document).ready(function() {	
	...
	var submitString = 'http://tekstur.dk/kn/lesing_skrivesjek/submituser.php?nocache='+Math.random();
...
function getTestXML(){
	var submitString = 'lesingskrivesjek_content.xml?nocache='+Math.random();
...
function checkAnswers(){
    ...
    var submitString = 'http://tekstur.dk/kn/lesing_skrivesjek/submititem.php?uid=' + uid + '&itemid=' + $XML.find("kntest").find('opgave').eq(currentQuestionIndex).attr("id") +'&answer='+ selectedAnswer +'&correct='+ answerScore +'&time='+ questionTime +'&totaltime='+ (new Date().getTime()-questionsStartTime) +'&nocache=' + Math.random();
...
function showResult(){
	...
	if(totalScore > secondLimit){//grøn (57 for regnesjekken)
		...
		$(".feedback.read-write h2").html("Basert på din vurdering er du på nivå 3 av 3 i denne sjekken.");
		$(".feedback.read-write p").html("Å kunne lese og skrive på nivå 3 innebærer å beherske et mangfold av tekster. Du leser og skriver på en selvstendig, kritisk og strategisk måte tilpasset situasjon og mål.");

		// $("#printbtn").attr("href",'http://tekstur.dk/selvtest16/rapport.php?cat=3&id='+rapportScoreBTOA);
        totalScore = 80;
	}else if(totalScore > firstLimit){//GUL (28 for regnesjekken)
        $("#result .score").html("NIVÅ:<span>2</span>");
		$(".feedback.read-write h2").html("Basert på din vurdering er du på nivå 2 av 3 i denne sjekken.");
		$(".feedback.read-write p").html("Å kunne lese og skrive på nivå 2 innebærer å kunne lese og skrive oversiktlige tekster. Du kan lese og skrive i de fleste situasjoner.");
        totalScore = 64;
		// $("#printbtn").attr("href",'http://tekstur.dk/selvtest16/rapport.php?cat=2&id='+rapportScoreBTOA);
	}else{//RØD
        $("#result .score").html("NIVÅ:<span>1</span>");
		$(".feedback.read-write h2").html("Basert på din vurdering er du på nivå 1 av 3 i denne sjekken.");
		$(".feedback.read-write p").html("Å kunne lese og skrive på nivå 1 innebærer å kunne lese og skrive korte og kjente tekster.");
        totalScore = 32;
...
function scaleScore() {
	/*if(totalScore <= firstLimit){
		return 0.32*(totalScore/firstLimit);
	}else if(totalScore <= secondLimit){
		return 0.33 + (0.13 * (((0-firstLimit)+totalScore)/(secondLimit-firstLimit)));
	}else{
		return Math.min(.785, 0.47+(0.315*(((0-secondLimit))+totalScore)/(topLimit-secondLimit)));
	}*/
    if(totalScore <= firstLimit){
		return 0.31;
	}else if(totalScore <= secondLimit){
...
function getPDF(){
    ...
    if(totalScore > top33){
        window.open('static/pdf/lesing_skrivesjek_3.pdf','_blank');
    }else if(totalScore > top32){
        window.open('static/pdf/lesing_skrivesjek_3.pdf','_blank');
    }else if(totalScore > top31){
        window.open('static/pdf/lesing_skrivesjek_3.pdf','_blank');
    }else if(totalScore > top23){
        window.open('static/pdf/lesing_skrivesjek_2.pdf','_blank');
    }else if(totalScore > top22){
        window.open('static/pdf/lesing_skrivesjek_2.pdf','_blank');
    }else if(totalScore > top21){
        window.open('static/pdf/lesing_skrivesjek_2.pdf','_blank');
    }else if(totalScore > top13){
        window.open('static/pdf/lesing_skrivesjek_1.pdf','_blank');
