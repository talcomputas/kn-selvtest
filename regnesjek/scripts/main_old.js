...
$(document).ready(function() {	
	...
	var submitString = 'http://tekstur.dk/kn/regnesjek/submituser.php?nocache='+Math.random();
...
function getTestXML(){
	var submitString = 'regnesjek_content.xml?nocache='+Math.random();
...
function checkAnswers(){
    ...
    var submitString = 'http://tekstur.dk/kn/regnesjek/submititem.php?uid=' + uid + '&itemid=' + $XML.find("kntest").find('opgave').eq(currentQuestionIndex).attr("id") +'&answer='+ selectedAnswer +'&correct='+ answerScore +'&time='+ questionTime +'&totaltime='+ (new Date().getTime()-questionsStartTime) +'&nocache=' + Math.random();
...
function showResult(){
	...
	if(totalScore > secondLimit){//grøn (57 for regnesjekken)
		...
		$(".feedback.calc h2").html("Basert på din vurdering er du på nivå 3 av 3 i denne sjekken.");
		$(".feedback.calc p").html("Å regne på nivå 3 innebærer å anvende sammensatt matematisk informasjon og kunnskap  i nødvendige situasjoner i hverdagen.");
        totalScore=70;
		...
		$(".feedback.calc h2").html("Basert på din vurdering er du på nivå 2 av 3 i denne sjekken.");
		$(".feedback.calc p").html("Å regne på nivå 2 innebærer å bruke regning i forskjellige situasjoner, bearbeide informasjon som inneholder tall og å ha et bevisst forhold til ulike regneoperasjoner.");
        totalScore=56;
		// $("#printbtn").attr("href",'http://tekstur.dk/selvtest16/rapport.php?cat=2&id='+rapportScoreBTOA);
	}else{//RØD
        $("#result .score").html("NIVÅ:<span>1</span>");
		$(".feedback.calc h2").html("Basert på din vurdering er du på nivå 1 av 3 i denne sjekken.");
		$(".feedback.calc p").html("Å regne på nivå 1 innebærer å bruke enkel regning i kjente sammenhenger og forstå enkel informasjon som inneholder tall.");
        totalScore=28;
