...
function drawQuestion(){
    ...
    if(calc.length>0) {
        $(".questionnaire .task .question").append("<a class='btn inverted' href='#' onClick='openCalculator()'>Ã…pne kalkulator</a>")
    }
    ...
    if(slider.length) {
        $(".questionnaire .task .answers").append('<div>Dra for Ã¥ avgi ditt svar:</div>');
    ...
    for(var i=0; i < finalAnswerArray.length; i++) {
        ...
        if(type == "test") {
            if(correctAnswer != answer) {
                ...
                mark = "<div class='mark wrong-answer'>âœ–</div>";
                questionClass = "mark"
            }
            else {
                answer = "<div class='correct-answer'>"+answer+"</div>";
                mark = "<div class='mark correct-answer'>âœ”</div>";
                questionClass = "mark"
            }
        } else {
...
function checkAnswers(){
    ...
    var submitString = 'submititem.php?uid=' + uid + '&itemid=' + question.attr("id") +'&answer='+ selectedAnswer +'&correct='+ answerScore +'&time='+ questionTime +'&totaltime='+ (new Date().getTime()-questionsStartTime) +'&nocache=' + Math.random();
...
function showResult(){
	...
	if(totalScore > secondLimit){//grÃ¸n (57 for regnesjekken)
	...
	// 0-0.32 = rÃ¸d | 0.33 - 0.46 = gul  |  0.47 - 0.785 = grÃ¸n
...
function scaleScore() {
    ...
    if(totalScore >= firstLimit) {
        ...
        var score2 = ((totalScore > secondLimit) ? secondLimit : totalScore) - firstLimit;
