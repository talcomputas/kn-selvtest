...
if($stmt = $mysqli->prepare($sql)) {
	...
	if($result->num_rows >0 && ($data = $result->fetch_assoc())){
		...
		$sql2 = "INSERT INTO itemdata (id,uid,itemid,answer,correct,time,totaltime,datecreated) VALUES(?,?,?,?,?,?,?,NOW());";
		$cid = uuid();
		$stmt2 = $mysqli->prepare($sql2);
		$stmt2->bind_param("sssssss",$cid,$uid,$itemid,$answer,$correct,$time,$totaltime);
		$stmt2->execute();
		$stmt2->close();
