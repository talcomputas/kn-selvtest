...
var printGeneratedPDF_test = function() {
    ...
    for(var i=0; i < finalAnswerArray.length; i++) {
        ...
        doc.setDrawColor(200);
        doc.line(18, 51 + j*25, 193, 51 + j*25);

        if(correctAnswer == answerGiven) {
            doc.alttext(20, 65 + j*25,"✔",18,"#089a00");
        }
        else {
            doc.alttext(20, 65 + j*25,"✖",18,"#c93f2b");
        }
        
        doc.text(30, (60 + j*25), question) 
        doc.setTextColor(34, 116, 185);

        if(correctAnswer == answerGiven) {
            doc.setTextColor(8, 154, 43);
            doc.text(160, (60 + j*25), answerGiven);
        }
        else {
            doc.setTextColor(201, 63, 43);
            doc.text(160, (60 + j*25), splitter(getString("YouAnswered")+": "+ answerGiven,20) );
            doc.setTextColor(34, 116, 185);
            doc.text(160, (66 + j*25), splitter(getString("RightAnswer")+": " + correctAnswer,20));
        }
        j++;
    }
