...
$(document).ready(function() {	
	...
	var submitString = 'http://tekstur.dk/kn/lesetest/submituser.php?nocache='+Math.random();
...
function submitItem(){	
    ...
    var submitString = 'http://tekstur.dk/kn/lesetest/submititem.php?uid=' + uid + '&itemid=' + submitArray[0] +'&answer='+ submitArray[1] +'&correctanswer=' + submitArray[2] +'&correct='+ submitArray[3] +'&time='+ taskTime +'&totaltime='+ (new Date().getTime()-testStartTime) +'&ver='+ xmlVersion + '&timeout=' + testOver + '&nocache=' + Math.random();
...
function setupResult(){
	...
	if(finalScore >= 4){//grøn
		...
		$(".feedback.reading p").html("Resultater over 4 viser at du er en god leser. Du leser vanlige tekster både raskt og nøyaktig.");
		...
		$(".feedback.reading p").html("Resultatet tyder på at leseferdighetene dine er ganske gode, men noen tekster kan være vanskelige og ta tid å lese for deg.");
		...
		$(".feedback.reading p").html("Resultatet ditt tyder på at mange tekster kan være vanskelige og ta lang tid å lese for deg. Du kan ha nytte av å øve mer på lesing.");
